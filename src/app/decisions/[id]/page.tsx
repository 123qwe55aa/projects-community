...
import { decisions, projects, candidates, conversations, decisionLinks, adoptionSnapshots, recommendations } from '@/db/schema';
...
export default async function DecisionDetailPage({
  ...
  // Fetch current adoption snapshot (if decision is decided)
  const currentAdoptions = await db
    .select()
    .from(adoptionSnapshots)
    .where(eq(adoptionSnapshots.decisionId, id));

  const currentAdoption = currentAdoptions.find((a) => a.isCurrent);

  // Find the adopted candidate if there is one
  let adoptedCandidate: typeof decisionCandidates[number] | null = null;
  if (currentAdoption) {
    const found = decisionCandidates.find((c) => c.id === currentAdoption.candidateId);
    if (found) adoptedCandidate = found;
  }

  // Get project info if decision is scoped to a project
  const linkedProject = decision.projectId
    ? (await db.select().from(projects).where(eq(projects.id, decision.projectId)))[0] ?? null
    : null;

  // Also check decision_links for project
  const links = await db
    .select({ projectId: decisionLinks.projectId })
    .from(decisionLinks)
    .where(eq(decisionLinks.decisionId, id));

  const linkedProjectFromLink = !decision.projectId && links.length > 0
        ...
        {currentAdoption && adoptedCandidate && (
          <div className="rounded-lg border border-green-800 bg-green-950/30 p-4 space-y-2">
            ...
            {currentAdoption.reasoning && (
              <p className="text-sm text-green-300/80">{currentAdoption.reasoning}</p>
            )}
          </div>
        )}
      </div>
          ...
          <div className="space-y-2">
            {decisionCandidates.map((candidate) => (
              ...
              >
                ...
                {currentAdoption && currentAdoption.candidateId === candidate.id && (
                  <span className="shrink-0 ml-3 rounded-full border border-green-700 bg-green-900/40 px-2 py-0.5 text-xs text-green-400">
                    Adopted
                  </span>
                )}
              </div>
